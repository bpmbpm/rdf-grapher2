...
function getFilterConfig(mode) {
    Logging.entered('validation', 'getFilterConfig');
    if (mode === 'base') {
        return FilterBase;
    }
...
function validateVAD(quads, prefixes) {
    Logging.entered('validation', 'validateVAD');
    const errors = [];
    quads.forEach((quad, index) => {
        ...
        const predicateAllowed = VAD_ALLOWED_PREDICATES.some(allowed => predicateUri === allowed || predicateLabel === allowed);
        if (!predicateAllowed) {
        ...
        if (typePredicates.includes(predicateUri) || typePredicates.includes(predicateLabel)) {
            ...
            const typeAllowed = VAD_ALLOWED_TYPES.some(allowed => typeUri === allowed || typeLabel === allowed);
            if (!typeAllowed) {
                const subjectLabel = getPrefixedName(quad.subject.value, prefixes);
                errors.push({
...
function formatVADErrors(errors) {
    Logging.entered('validation', 'formatVADErrors');
...
function loadExampleTurtle() {
    Logging.entered('examples/core', 'loadExampleTurtle');
...
function loadExampleVAD() {
    Logging.entered('examples/core', 'loadExampleVAD');
...
function loadExampleNTriples() {
    Logging.entered('examples/core', 'loadExampleNTriples');
...
function loadExampleNQuads() {
    Logging.entered('examples/core', 'loadExampleNQuads');
...
function loadExampleTriG() {
    Logging.entered('examples/core', 'loadExampleTriG');
...
function loadExample() {
    Logging.entered('examples/core', 'loadExample');
    loadExampleTurtle();
}
// ============================================================================
...
function getLocalName(uri) {
    Logging.entered('examples/core', 'getLocalName');
...
function getPrefixedName(uri, prefixes) {
    Logging.entered('examples/core', 'getPrefixedName');
    if (typeof uri !== 'string')
        return String(uri);
...
function escapeDotString(str) {
    Logging.entered('examples/core', 'escapeDotString');
    return str.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
}
function generateNodeId(value) {
    Logging.entered('examples/core', 'generateNodeId');
    let hash = 0;
...
function generateVadNodeId(uri, prefixes) {
    Logging.entered('examples/core', 'generateVadNodeId');
...
function isNameOrLabelPredicate(predicateLabel) {
    Logging.entered('examples/core', 'isNameOrLabelPredicate');
...
function escapeHtmlLabel(str) {
    Logging.entered('examples/core', 'escapeHtmlLabel');
...
function escapeHtml(text) {
    Logging.entered('examples/core', 'escapeHtml');
    const div = document.createElement('div');
    div.textContent = text;
    return div.innerHTML;
}
function wrapTextByWords(text, maxLength) {
    Logging.entered('examples/core', 'wrapTextByWords');
...
function formatLabelWithWrap(label, maxLength, isBold = false) {
    Logging.entered('examples/core', 'formatLabelWithWrap');
    const lines = wrapTextByWords(label, maxLength);
    const needsWrap = lines.length > 1;
    if (!needsWrap) {
        if (isBold)
            return `<B>${escapeHtmlLabel(label)}</B>`;
...
function getNodeStyle(nodeUri, isLiteral, isBlankNode) {
    Logging.entered('interface', 'getNodeStyle');
    if (currentMode === 'base') {
        if (isLiteral)
    ...
    if (currentMode === 'aggregation') {
        if (isBlankNode)
            return AggregationNodeStyles['BlankNodeStyle'].dot;
...
function getEdgeStyle(predicateUri, predicateLabel) {
    Logging.entered('interface', 'getEdgeStyle');
    if (currentMode === 'base')
...
function buildNodeTypesCache(quads, prefixes) {
    Logging.entered('interface', 'buildNodeTypesCache');
    nodeTypesCache = {};
    const typePredicates = [
        'http://www.w3.org/1999/02/22-rdf-syntax-ns#type',
        'rdf:type',
...
// ============================================================================
function applyZoom() {
    Logging.entered('interface', 'applyZoom');
...
function zoomIn() {
    Logging.entered('interface', 'zoomIn');
    if (currentScale < 3.0) {
        currentScale += 0.1;
        applyZoom();
    }
...
function zoomOut() {
    Logging.entered('interface', 'zoomOut');
    if (currentScale > 0.1) {
        currentScale -= 0.1;
        applyZoom();
    }
...
function zoomReset() {
    Logging.entered('interface', 'zoomReset');
    currentScale = 1.0;
    applyZoom();
}
function zoomFit() {
    Logging.entered('interface', 'zoomFit');
...
function closePropertiesPanel(panelId) {
    Logging.entered('interface', 'closePropertiesPanel');
...
function closeAllPropertiesPanels() {
    Logging.entered('interface', 'closeAllPropertiesPanels');
...
function getNodeProperties(nodeUri) {
    Logging.entered('interface', 'getNodeProperties');
    const properties = [];
...
function showNodeProperties(nodeUri, nodeLabel) {
    Logging.entered('interface', 'showNodeProperties');
    const container = document.getElementById('properties-panels-container');
    if (!container)
        return;
...
function bringPanelToFront(panel) {
    Logging.entered('interface', 'bringPanelToFront');
    let maxZIndex = 1000;
...
function startDragPanel(event, panelId) {
    Logging.entered('interface', 'startDragPanel');
...
function dragPanel(event) {
    Logging.entered('interface', 'dragPanel');
    if (!draggedPanel)
        return;
...
function stopDragPanel() {
    Logging.entered('interface', 'stopDragPanel');
    draggedPanel = null;
...
function addNodeClickHandlers() {
    Logging.entered('interface', 'addNodeClickHandlers');
...
function handleNodeClick(event) {
    Logging.entered('interface', 'handleNodeClick');
...
function togglePredicateFilter(predicateUri, predicateLabel, isVisible) {
    Logging.entered('interface', 'togglePredicateFilter');
    if (isVisible) {
...
function selectAllFilters() {
    Logging.entered('interface', 'selectAllFilters');
    activeFilters = [];
    displayFilters();
    revisualize();
}
function deselectAllFilters() {
    Logging.entered('interface', 'deselectAllFilters');
    activeFilters = allPredicates.flatMap(p => [p.uri, p.label]);
    displayFilters();
    revisualize();
}
async function revisualize() {
    Logging.entered('interface', 'revisualize');
...
function rdfToDotVAD(quads, prefixes = {}) {
    Logging.entered('conversion', 'rdfToDotVAD');
    ...
    quads.forEach(quad => {
        ...
        const isProcess = subjectTypes.some(t => t === 'vad:Process' || t === 'http://example.org/vad#Process');
        if (isProcess) {
        ...
        const isExecutorGroup = subjectTypes.some(t => t === 'vad:ExecutorGroup' || t === 'http://example.org/vad#ExecutorGroup');
        if (isExecutorGroup) {
    ...
    quads.forEach(quad => {
        ...
        const objectValue = quad.object.value;
        const subjectTypes = nodeTypesCache[subjectUri] || [];
        const isExecutor = subjectTypes.some(t => t === 'vad:Executor' || t === 'http://example.org/vad#Executor');
    ...
    visibleProcesses.forEach((processInfo, uri) => {
        ...
        if (processInfo.executorGroup && executorGroups.has(processInfo.executorGroup)) {
            ...
            const executorNamesList = group.executors.map(exUri => executorNames.get(exUri) || getPrefixedName(exUri, prefixes));
...
function rdfToDotAggregation(quads, prefixes = {}) {
    Logging.entered('conversion', 'rdfToDotAggregation');
    buildNodeTypesCache(currentQuads, prefixes);
    nodeLabelToUri = {};
    const nodes = new Map();
    const edges = [];
    ...
    nodes.forEach((nodeInfo, value) => {
        ...
        for (const lit of nameLabelLiterals) {
            if (addedLines >= MaxAggregationParams)
                break;
            htmlLabel += '<BR/>';
...
function rdfToDot(quads, prefixes = {}) {
    Logging.entered('conversion', 'rdfToDot');
    if (currentMode === 'aggregation') {
...
function showLoading() {
    Logging.entered('view', 'showLoading');
    const output = document.getElementById('output');
...
function showError(message) {
    Logging.entered('view', 'showError');
    const output = document.getElementById('output');
    const resultContainer = document.getElementById('result-container');
    resultContainer.style.display = 'block';
...
function showValidationError(message) {
    Logging.entered('view', 'showValidationError');
    const output = document.getElementById('output');
    const resultContainer = document.getElementById('result-container');
    resultContainer.style.display = 'block';
...
function displayPrefixes(prefixes) {
    Logging.entered('view', 'displayPrefixes');
...
function displayLegend() {
    Logging.entered('view', 'displayLegend');
...
function updateModeDescription() {
    Logging.entered('view', 'updateModeDescription');
...
function toggleSparqlPanel() {
    Logging.entered('view', 'toggleSparqlPanel');
...
async function visualize() {
    Logging.entered('workflow', 'visualize');
...
function svgToPng(svgString) {
    Logging.entered('workflow', 'svgToPng');
...
function downloadSVG() {
    Logging.entered('workflow', 'downloadSVG');
...
async function downloadPNG() {
    Logging.entered('workflow', 'downloadPNG');
    if (!currentSvgElement) {
        alert('Сначала визуализируйте RDF данные');
        return;
    }
...
function openInNewWindowLdfFi() {
    Logging.entered('workflow', 'openInNewWindowLdfFi');
...
function openInNewWindowGitHub() {
    Logging.entered('workflow', 'openInNewWindowGitHub');
    // Получаем входные данные
    const rdfInput = document.getElementById('rdf-input').value.trim();
    const inputFormat = document.getElementById('input-format').value;
    const outputFormat = document.getElementById('output-format').value;
...
function openInNewWindowGraphvizOnline() {
    Logging.entered('workflow', 'openInNewWindowGraphvizOnline');
...
// @ts-nocheck — Comunica загружается как браузерный UMD-модуль без официальных глобальных типов.
async function initSparqlEngine() {
    Logging.entered('sparql', 'initSparqlEngine');
    if (!currentStore) {
...
async function executeSparqlQuery() {
    Logging.entered('sparql', 'executeSparqlQuery');
...
function highlightNodeFromSparqlResult(rowElement) {
    Logging.entered('sparql', 'highlightNodeFromSparqlResult');
...
function resetSparqlQuery() {
    Logging.entered('sparql', 'resetSparqlQuery');
...
function parseUrlParams(search = window.location.search, hash = window.location.hash) {
    Logging.entered('initialization', 'parseUrlParams');
    ...
    if (rdf !== null)
        params.rdf = rdf;
    if (from !== null)
        params.from = reverseFormatMapping[from] ?? 'turtle';
    if (to === 'svg' || to === 'png')
        params.to = to;
...
function applyUrlParams(params) {
    Logging.entered('initialization', 'applyUrlParams');
    if (params.rdf === undefined)
        return;
    byId('rdf-input').value = params.rdf;
    if (params.from)
        byId('input-format').value = params.from;
    if (params.to)
        byId('output-format').value = params.to;
    if (params.mode)
        byId('visualization-mode').value = params.mode;
    updateModeDescription();
    window.setTimeout(() => void RdfGrapher.workflow.visualize(), 100);
}
function publishGroup(group, api) {
    Logging.entered('initialization', 'publishGroup');
    const instrumented = Logging.instrument(group, api);
    RdfGrapher[group] = instrumented;
    Object.assign(window, instrumented);
}
function main() {
    Logging.entered('initialization', 'main');
